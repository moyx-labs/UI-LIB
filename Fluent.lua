...
local a,b={
    ...
    return u 
        ...
        return function(Name, Parent, Callback)
            local Dropdown = {
                Values = {},
                Selected = nil,
            ...
            Dropdown.Frame = New("Frame", {
                ...
                New("Frame", {
                    ...
                    ThemeTag = { BackgroundColor3 = "Dropdown" },
                }, {
                    ...
                    Dropdown.SelectedText = New("TextLabel", {
            ...
            Dropdown.OptionHolder = New("Frame", {
                Size = UDim2.new(1, -10, 0, 0),
                Position = UDim2.fromOffset(5, 38),
                BackgroundTransparency = 0.88,
                Visible = false,
                ThemeTag = { BackgroundColor3 = "DropdownOptionHolder" },
            }, {
                New("UICorner", { CornerRadius = UDim.new(0, 4) }),
                New("UIStroke", {
                    Transparency = 0.5,
                ...
                Dropdown.OptionList = New("ScrollingFrame", {
                    ...
                    New("UIListLayout", {
                        ...
                        Padding = UDim2.new(0, 4),
                    }),
                    New("UIPadding", {
                        PaddingTop = UDim2.new(0, 4),
                        PaddingBottom = UDim2.new(0, 4),
                    }),
                }),
            })
    
            Dropdown.OptionHolder.Parent = Dropdown.Frame
    
            local OptionMotor = Flipper.GroupMotor.new({ Open = 0, Rotate = 0 })
            OptionMotor:onStep(function(values)
                Dropdown.OptionHolder.Size = UDim2.new(1, -10, 0, values.Open)
                Dropdown.Frame:FindFirstChildWhichIsA("ImageLabel").Rotation = values.Rotate
            end)
    
            Creator.AddSignal(Dropdown.Button.MouseButton1Click, function()
                local isOpen = Dropdown.OptionHolder.Visible
                Dropdown.OptionHolder.Visible = true
                OptionMotor:setGoal({
                    Open = Spring(isOpen and 0 or Dropdown.OptionList.CanvasSize.Y.Offset + 8, { frequency = 5 }),
                    Rotate = Spring(isOpen and 0 or 180, { frequency = 5 }),
                })
                if not isOpen then
                    task.wait(0.1)
                    Dropdown.OptionHolder.Visible = true
            ...
            function Dropdown.Refresh()
                for _, child in ipairs(Dropdown.OptionList:GetChildren()) do
                ...
                Dropdown.OptionList.CanvasSize = UDim2.new(0, 0, 0, count * 24 + 8)
                ...
                for i, value in ipairs(Dropdown.Values) do
                    ...
                    Option.Parent = Dropdown.OptionList
    
                    Creator.AddSignal(Option.MouseButton1Click, function()
                        Dropdown.Selected = value
                        Dropdown.SelectedText.Text = tostring(value)
                        Dropdown.Callback(value)
                        OptionMotor:setGoal({
                            Open = Spring(0, { frequency = 5 }),
                            Rotate = Spring(0, { frequency = 5 }),
                        })
                        task.delay(0.3, function()
                            Dropdown.OptionHolder.Visible = false
                        end)
                ...
                if Dropdown.Selected and table.find(Dropdown.Values, Dropdown.Selected) then
                    Dropdown.SelectedText.Text = tostring(Dropdown.Selected)
                else
                    Dropdown.SelectedText.Text = "Select Option"
